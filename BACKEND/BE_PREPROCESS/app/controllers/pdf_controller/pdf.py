...
from schemas.pdf import DocumentMetadata, PDFRequest, PDFResponse
...
async def send_webhook_payload(succeeded_list: List[str], failed_list: List[str], error_messages: List[dict]):
    try:
        ...
        payload = {
            "succeeded_pdfs": succeeded_list,
            "failed_pdfs": failed_list,
            "error_pdfs": error_messages,
        }
...
async def process_urls(doc_metadata: List[DocumentMetadata], s3_client: AsyncS3Client):
    ...
    try:
        pipeline_url = RAGPreprocessingPipeline(type="pdfs")
        ...
        try:
            ...
            for idx, doc in enumerate(documents):
                ...
                name = get_filename_from_pdf(file or "")
                filename = f"processed_rag_pdf_{name}_{idx}.pdf"
                with tempfile.NamedTemporaryFile(delete=False, mode="w", encoding="utf-8", suffix=".pdf") as md_file:
                    md_file.write(str(content_md))
                    md_file_path = md_file.name

                await s3_client.put_object(
                    S3Input(bucket_name=BUCKET_NAME, object_name=filename, file_path=md_file_path),
                    extra_args={"Metadata": metadata},
                )
                logger.info("Markdown content uploaded to S3 with metadata", s3_key=filename)
                try:
            ...
            return exception_handler.handle_exception(
                ...
                extra={"error": "Processing pipeline failed", "pdfs": paths},
            )
        ...
        return exception_handler.handle_exception(e=str(e), extra={"error pdfs": paths if paths else []})
    ...
    if not succeeded_list and not failed_list:
        ...
        final_message = "Processing failed for all requested PDFS."
    elif failed_list:
...
@pdf_router.post("/pdfs-processing/", response_model=PDFResponse, status_code=status.HTTP_200_OK)
async def url_processing(
    ...
    """
    Accepts user-submitted pdfs and processes them in the background. Uploads to S3 and notifies webhook.
    """
    exception_handler = ExceptionHandler(
        logger=logger.bind(),
    ...
    try:
        ...
        if not request.pdfs:
            return exception_handler.handle_bad_request(
                e="No PDFS provided in the request.",
                extra={"payload": request.model_dump()},
            )

        #  validation for pdfs
        for url_metadata in request.pdfs:
            if not url_metadata.source or not url_metadata.source.endswith('.pdf'):
                return exception_handler.handle_bad_request(
                    e=f"Invalid URL format: {url_metadata.source}",
                    extra={"payload": request.model_dump()},
                )

        background_tasks.add_task(process_urls, request.pdfs, s3_client)

        logger.info("Background task added for URL processing", url_count=len(request.pdfs))
        return PDFResponse(
            message=f"Submitted {len(request.pdfs)} URLs for processing. Check your webhook for results."
        )
    except FileNotFoundError as e:
        return exception_handler.handle_not_found_error(
            e=str(e), extra={"error urls": [url.source for url in request.pdfs] if request.pdfs else []}
        )
    except Exception as e:
        return exception_handler.handle_exception(
            e=str(e), extra={"error urls": [url.source for url in request.pdfs] if request.pdfs else []}
