...
class PDFExpertPreprocessingPipeline:
    def __init__(self, config: BaseConfiguration = APP_CONFIG):
        ...
        self.vector_store = create_expert_store(configuration=config, embedding_model=self.embedding_model)
    ...
    async def upload_to_s3(self, file: UploadFile, metadata: Dict[str, Any], s3_client: AsyncS3Client) -> str:
        ...
        try:
            ...
            s3_key = f"knowledge_store/expert_knowledge_pdf/{file.filename}"
            ...
            for idx, file in enumerate(pdf_files):
                try:
                    ...
                    s3_metadata = {
                        ...
                        "type": metadatas[idx].type if metadatas and idx < len(metadatas) else "EXPERT_KNOWLEDGE",
